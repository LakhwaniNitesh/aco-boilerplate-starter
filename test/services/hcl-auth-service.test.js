...
class HCLAuthServiceMock {
  ...
  isTokenExpired() {
    const expiry = this.tokenExpiry || parseInt(sessionStorage.getItem('hcl_token_expiry'), 10);
...
describe('HCLAuthService Unit Tests', () => {
  ...
  describe('login()', () => {
    it('should throw error when email is missing', async () => {
      await expect(auth.login('', 'password')).rejects.toThrow('Email and password required');
    });

    it('should throw error when password is missing', async () => {
      await expect(auth.login('test@example.com', '')).rejects.toThrow('Email and password required');
    });
    ...
    it('should store token in sessionStorage', async () => {
      ...
      expect(sessionStorage.setItem).toHaveBeenCalledWith('hcl_token', 'test-token-123');
    ...
    it('should throw error on invalid credentials', async () => {
      ...
      await expect(auth.login('invalid@test.com', 'wrongpassword')).rejects.toThrow('Login failed');
    });
  ...
  describe('logout()', () => {
    ...
    it('should clear sessionStorage', () => {
      ...
      expect(sessionStorage.removeItem).toHaveBeenCalledWith('hcl_token_expiry');
    });
