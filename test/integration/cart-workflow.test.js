...
describe("E2E Cart Workflow Integration Tests", () => {
  ...
  describe("Authentication & Cart Access", () => {
    ...
    it("should reject invalid credentials", async () => {
      await expect(
        backend.login("test@example.com", "wrongpassword"),
      ).rejects.toThrow("Invalid credentials");
    });

    it("should reject unknown user", async () => {
      await expect(
        backend.login("unknown@example.com", "password123"),
      ).rejects.toThrow("Invalid credentials");
    });
  });
  ...
  describe("Checkout Process", () => {
    ...
    it("should prevent checkout with empty cart", async () => {
      ...
      await expect(backend.checkout(token, "cart-id")).rejects.toThrow(
        "Cart is empty",
      );
    });

    it("should require authentication for checkout", async () => {
      await expect(backend.checkout(null, "cart-id")).rejects.toThrow(
        "Unauthorized",
      );
    });
  ...
  describe("Complete User Journey", () => {
    ...
    it("should handle multiple users with separate carts", async () => {
      ...
      backend.users["user2@example.com"] = {
        password: "pass456",
        token: user2Token,
      };
  ...
  describe("Performance Scenarios", () => {
    it("should handle cart with many items", async () => {
      const token = (await backend.login("test@example.com", "password123"))
        .token;
