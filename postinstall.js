...
fs.readdirSync("node_modules/@dropins", { withFileTypes: true }).forEach(
  (file) => {
...
[
  ...
  {
    from: "@adobe/magento-storefront-events-sdk/dist/index.js",
    to: "commerce-events-sdk.js",
  },
].forEach((file) => {
  fs.copyFileSync(
      ...
      try {
        ...
        Object.keys(packageLock.packages).forEach((packageName) => {
          ...
          if (
            ...
            packageInfo.resolved.includes("artifactory")
          ) {
            console.warn(
...
function checkSourceMaps() {
  ...
  if (
    !fs.existsSync(hlxIgnorePath) ||
    !fs.readFileSync(hlxIgnorePath, "utf-8").includes("*.map")
  ) {
    console.info(
