...
export default async function decorate(block) {
  ...
  const storeConfig = {
    type: "eds",
    environmentType: getConfigValue("analytics.environment").includes(
      "Production",
    )
      ? ""
      : "testing",
    ...
    config: {
      ...
      addToCart: async (...args) => {
        const { addProductsToCart } =
          await import("../../scripts/__dropins__/storefront-cart/api.js");
  ...
  if (type !== "search") {
    storeConfig.config.categoryName = document.querySelector(
  ...
  if (rootElement) {
    provider.render(ProductListingPage, { storeConfig })(rootElement);
  } else {
