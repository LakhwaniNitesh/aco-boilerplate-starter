 ...
 *
 * Usage: node api/test-auth-load.js [concurrency] [iterations]
 * Example: node api/test-auth-load.js 10 100
 */
    ...
    const avgResponseTime =
      this.results.responseTimes.length > 0
        ? (this.results.responseTimes.reduce((a, b) => a + b, 0) /
            this.results.responseTimes.length).toFixed(2)
        : 0;
  ...
  Expiry Window:      25 minutes (1500 seconds)
    ...
    if (this.results.tokens.length > 0) {
      ...
      this.results.tokens.slice(0, 3).forEach((t) => {
        console.log(
          `    [${t.index}] ${t.token} (expires: ${t.expiresIn}s)`
        );
      });
    }
