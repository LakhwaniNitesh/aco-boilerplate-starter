...
const TEST_PRODUCTS = [
  ...
  { partNumber: 'SKU-456', name: 'Premium Product', price: 299.99, quantity: 2 },
];
...
async function testPhase2(previousToken = null) {
  ...
  try {
    ...
    if (res.status === 200 && res.body.success) {
      ...
      failure('Unexpected response', `Status: ${res.status}, Body: ${JSON.stringify(res.body)}`);
    }
  } catch (error) {
...
async function testPhase3(accessToken = null) {
  ...
  try {
    ...
    if (res.status === 200 && res.body.success && res.body.cart) {
      ...
      failure('Add to cart failed', `Status: ${res.status}, Message: ${res.body.error}`);
    }
  } catch (error) {
  ...
  try {
    ...
    if (res.status === 200 && res.body.success && res.body.cart) {
      ...
      if (res.body.cart.items?.length === 0) {
        ...
        failure('Cart still has items', `Items: ${res.body.cart.items?.length}`);
      }
    } else {
...
async function displayResults() {
  ...
  log(`Passed: ${testResults.passed}`, testResults.failed === 0 ? 'green' : 'yellow');
