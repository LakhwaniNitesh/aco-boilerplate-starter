...
const log = {
  ...
  header: (msg) =>
    console.log(
      `${colors.bright}${colors.blue}║ ${msg.padEnd(38)} ║${colors.reset}`,
    ),
  footer: () =>
    console.log(
...
async function main() {
  ...
  for (const endpoint of endpoints) {
    ...
    if (result.error) {
      ...
      log.warn(
        `${endpoint} - 401 Unauthorized (endpoint exists but credentials invalid)`,
      );
    } else if (result.ok) {
  ...
  console.log(
    '✓ If you see "401 Unauthorized" = endpoint exists, credentials are wrong',
  );
